...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'icp_score') THEN
    ALTER TABLE leads ADD COLUMN icp_score INTEGER CHECK (icp_score >= 0 AND icp_score <= 100);
  END IF;
END $$;

-- 2. SDR Analyses Table (stores lead analysis results)
...
-- 3. Agent Runs Table (tracks agent execution jobs)
...
-- 4. Agent Tasks Table (individual tasks within a run)
...
-- 5. Update existing tables with agent-related fields
